...
public class BitwiseOperatorsDemo {
    ...
    // TODO: method converts a boolean array to a string of the form 01010 or 10101 (10 or -10)
    public static String negateBinaryBoolArray(boolean[] negativeBinary) {
        char[] charArr = new char[negativeBinary.length];
        byte i = 0;
        for (boolean bool: negativeBinary) {
            charArr[i] = bool ? '1' : '0';
            i++;
        }
        return String.valueOf(charArr);
...
//                System.out.println("new returnInt is : " + returnInt);
    ...
    public static boolean[] deepCopyBoolArr(boolean[] boolArr) {
        boolean[] newBoolArr = new boolean[boolArr.length];
        byte i = 0;
     ...
     public static boolean[] binaryAddition(boolean[] b1, boolean[] b2) {
        ...
        boolean[] longer = len1 >= len2 ? deepCopyBoolArr(b1): deepCopyBoolArr(b2);
        boolean[] shorter = len1 < len2 ? deepCopyBoolArr(b1): deepCopyBoolArr(b2);
        boolean[] l = longer;
        boolean[] s = shorter;
...
//        byte diff = (byte) ((len1 - len2 < 0 ? len2- len1 : len1 - len2) -1);
        ...
        if (needExtra) {
            ...
            biggerArray[0] = true;
            for(byte k = 1; k < length; k++) {
                biggerArray[k] = longer[k - 1];
            }
            return biggerArray;
        }
...
// Convert a binary integer representation to a boolean[].
    ...
    // Strings are positive when beginning with 0 and negative beginning with 1
    // ex  10 == 01010, -10 == 10101
    // Resulting bool arrays are positive when beginning with false, otherwise negative
    public static boolean[] binaryToBoolArray(String binaryStr) {
//        String binaryStr = String.valueOf(binary);
        byte length = (byte)binaryStr.length();
        boolean[] binaryArr = new boolean[length];
    ...
    // TODO: if isNegative is true then treats the boolean array like so:
    // If the array starts with 0, then it's positive and it can be printed out easily.
    // if the array starts with 1, then it's negative and it needs to be converted.
    public static int boolArrayToBinaryInt(boolean[] boolArr, boolean isNegative) {
        if (boolArr[0] && isNegative) {
    ...
    public static void main(String[] args)
    {
        ...
        boolean[] twentyFive = binaryToBoolArray(intToBinary(25)); // [true, true, false, false, true]
        boolean[] thirtyNine = binaryToBoolArray(intToBinary(39)); // [true, false, false, true, true, true]

//        //boolean[] ten                                             // 1010
//        boolean[] four = binaryToBoolArray(intToBinary(4)); // 100

//        System.out.println("MATH: " + Arrays.toString(binaryAddition(four, ten)));  //
//        System.out.println("INTEGER= " + boolArrayToBinaryInt(binaryAddition(four, ten)));

        System.out.println("MATH: " + Arrays.toString(binaryAddition(twentyFive, thirtyNine)));  // 1100000 should be 1000000 or 64
        System.out.println("INTEGER= " + boolArrayToBinaryInt(binaryAddition(twentyFive, thirtyNine)));

                                                                                                     //   128    64    32     16     8      4     2    1
        System.out.println("MATH: " + Arrays.toString(binaryAddition(hundred, hundredFiftySix)));  // [true, true, true, false, false, true, false, false]  = 128 + 64 + 32 + 4 = 228
        ...
        System.out.println("INTEGER= " + boolArrayToBinaryInt(binaryAddition(ten, ten)));
